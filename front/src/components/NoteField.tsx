...
const NoteField = ({ weekId }: NoteFieldProps) => {
  ...
  if (isLoading && !error) {
    return (
      <section className="note-bento-card flex flex-col gap-2 p-2 min-[480px]:p-3 min-[480px]:gap-3 max-[360px]:p-1.5 max-[360px]:gap-1.5 max-[360px]:rounded-[10px] rounded-xl bg-surface-glass border border-border-subtle shadow-glass backdrop-blur-[14px] min-h-0 h-full overflow-hidden">
        <header className="flex items-center justify-between gap-2 shrink-0">
          <span className="day-bento-date">Заметки</span>
        </header>
  ...
  if (error && !isLoading) {
    return (
      <section
        className="note-bento-card flex flex-col gap-2 p-2 min-[480px]:p-3 min-[480px]:gap-3 max-[360px]:p-1.5 max-[360px]:gap-1.5 max-[360px]:rounded-[10px] rounded-xl bg-surface-glass border border-border-subtle shadow-glass backdrop-blur-[14px] min-h-0 h-full overflow-hidden"
        role="alert"
      >
        <header className="flex items-center justify-between gap-2 shrink-0">
          <span className="day-bento-date">Заметки</span>
  ...
  return (
    <section className="note-bento-card flex flex-col gap-2 p-2 min-[480px]:p-3 min-[480px]:gap-3 max-[360px]:p-1.5 max-[360px]:gap-1.5 max-[360px]:rounded-[10px] rounded-xl bg-surface-glass border border-border-subtle shadow-glass backdrop-blur-[14px] min-h-0 h-full overflow-hidden">
      <header className="flex items-center justify-between gap-2 shrink-0">
        <span className="day-bento-date">Заметки недели</span>
        <button
          type="button"
          className="note-bento-action inline-flex items-center justify-center border border-border-subtle bg-[rgba(255,255,255,0.04)] text-text-secondary transition-all duration-[160ms] ease-linear hover:bg-[rgba(255,255,255,0.08)] hover:border-border-strong hover:-translate-y-px active:translate-y-0"
          onClick={handleNavigate}
      ...
      <button
        type="button"
        className={clsx(
          'flex-1 flex items-start w-full min-h-11 p-3 max-[360px]:p-2 rounded-[10px]',
          'bg-[var(--note-preview-bg)] border border-dashed border-border-subtle',
          'text-left transition-all duration-[160ms] ease-linear cursor-pointer overflow-hidden',
          'hover:bg-[var(--note-preview-bg-hover)] hover:border-border-accent',
          'focus-visible:bg-[var(--note-preview-bg-hover)] focus-visible:border-border-accent focus-visible:outline-none'
        )}
        onClick={handleNavigate}
        aria-label={
          noteContent
      ...
      >
        <p className="note-preview-text line-clamp-3 overflow-hidden text-ellipsis w-full">
