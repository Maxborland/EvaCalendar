...
const TaskForm: React.FC<TaskFormProps> = ({ initialData, weekId, dayOfWeek, onTaskSaved, onClose }) => {
  const [formData, setFormData] = useState({
    id: initialData?.id || uuidv4(), // ID is generated by UUID on frontend
    type: 'income' as 'income' | 'expense', // Default value
    title: '' as string | null, // Default value
    time: '',
    address: '',
    childId: null as string | null, // Добавлено childId
    childName: '', // Оставлено для отображения
    hourlyRate: 0,
    parentName: '',
    parentPhone: '',
    comments: '',
  ...
  useEffect(() => {
    if (initialData) {
      const { id, childName, childId, ...restOfInitialData } = initialData; // Изменено: добавлен childName и childId в деструктуризацию
      setFormData(prevData => ({
        ...
        childId: childId || null, // Устанавливаем childId
        childName: childName || '', // Используем childName из initialData
       ...
       // Если initialData содержит childId, то сразу устанавливаем selectedChildId
    ...
    const fetchChildren = async () => {
      try {
        ...
        setChildren(fetchedChildren);
          ...
          if (initialData?.childId) {
            setSelectedChildId(initialData.childId);
          } else if (initialData?.childName) { // Оставлено для обратной совместимости, если initialData приходит со старым форматом
            const foundChild = fetchedChildren.find(child => child.childName === initialData.childName);
            if (foundChild) {
              setSelectedChildId(foundChild.childId);
            }
          }
      } catch (error) {
    ...
    if (selectedChildId !== null) {
      const fetchChildData = async () => {
        try {
          ...
          setFormData((prevData) => ({
            ...prevData,
            childId: childData.childId, // Обновляем childId в formData
    ...
    try {
      ...
      if (formData.type === 'income') {
        dataToSave = {
          ...
          childId: selectedChildId, // Заменено childName на childId
            ...
            <>
              <div className="form-group">
                ...
                >
                  ...
                  {children.map((child) => (
                    <option key={child.childId} value={child.childId}> {/* child.id теперь string */}
                      {child.childName}
                    </option>
                  ))}
                </select>
